...
async def get_all(uow: UOWDep):
    res = await UnitsService().get_units(uow=uow)
    return res


@router.get(path="/{id}", response_model=UnitResponseSchema)
async def get_one(id: int, uow: UOWDep):
    res = await UnitsService().get_unit(uow=uow, params=dict(id=id))
    return res


@router.post("")
async def add(schema: UnitAddSchema, uow: UOWDep):
    id = await UnitsService().add_unit(uow=uow, schema=schema)
    return {"unit_id": id}


@router.patch("/{id}")
async def edit(id: int, schema: UnitEditSchema, uow: UOWDep):
    await UnitsService().edit_unit(uow=uow, params=dict(id=id), schema=schema)
    return {"ok": True}


@router.delete("/{id}")
async def delete(id: int, uow: UOWDep):
    await UnitsService().delete_unit(uow=uow, params=dict(id=id))
    return {"ok": True}